...
. To check the status of the installation, run the command `oc describe sub amq-streams`.  Under Conditions, ensure that the catalog sources are healthy and have no error messages:
+
[source,bash]
----
  Conditions:
